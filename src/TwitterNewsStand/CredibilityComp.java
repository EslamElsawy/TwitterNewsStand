...
//        out.close();
//    }
    public void SecondRun(Vector<Pair<Tweet, Integer>> v, HashMap<Integer, Cluster> myclusters) throws IOException {
        double val, clustCred = 0;
        for (Pair<Tweet, Integer> tweet : v) {
            clustCred = 0;
            val = checkTweetCredibility(tweet.getL(), myclusters.get(tweet.getR()).getMembers().size(), myclusters.get(tweet.getR()).getMembers());
            for (int i = 0; i < myclusters.get(tweet.getR()).getMembers().size(); i++) {
                if (myclusters.get(tweet.getR()).getMembers().get(i).getCredibility() == 0.0) {
                    val = checkTweetCredibility(myclusters.get(tweet.getR()).getMembers().get(i), myclusters.get(tweet.getR()).getMembers().size(), myclusters.get(tweet.getR()).getMembers());
                    myclusters.get(tweet.getR()).getMembers().get(i).setCredibility(val);
                }
            }
        }
        ...
        while (it.hasNext()) {
            ...
            for (Tweet tweet : pairs.getValue().getMembers()) {
                val = tweet.getCredibility();
                clustCred += val;

            }

            pairs.getValue().setCredibility(clustCred);
